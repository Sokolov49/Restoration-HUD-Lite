function MenuRenderer:show_node(node)
    ...
    local parameters = {
   	 font = "fonts/font_medium_shadow_mf", --tweak_data.menu.default_font,
   	 row_item_color = Color( 1, 0.95, 0.95, 0.95),
   	 row_item_hightlight_color = Color( 1, 1, 1, 1), --Color.red,
   	 row_item_blend_mode = "normal", -- "normal",
   	 font_size = 26, -- tweak_data.menu.default_font_size,
   	 node_gui_class = gui_class,
   	 spacing = node:parameters().spacing,
   	 marker_alpha = 0.1, -- 0.15 or 0.8
...
function MenuRenderer:_create_bottom_text()
	...
	local bh = CoreMenuRenderer.Renderer.border_height
	self._bottom_text:set_y( scaled_size.height - bh + 10) --2
